...
- CI: `cargo fmt`, `cargo test`, `oursql-node --features tls`, `cargo deny`
...
`crates/oursql-engine/tests/doc_check.rs` keeps the error catalog, keyword table, and OCHERED types aligned with the code.
